...
These rules ensure `plans/` documents function as **binding internal business guidance**, not informal notes. They apply to all repositories under the ai-lib organization and to partner repos that ship ai-lib–branded products.
...
3. **Product-specific internal plans** — `plans/` in individual repos (must not contradict 1–2)  
...
- PR to `main` with label `plans-minor`  
...
Template (add to org `.github` when available):

```markdown
## Portfolio alignment
- [ ] Reviewed against plans/PRODUCT_LINE_CHARTER.md (section: ___)
- [ ] Identity impact: none / documented in PR
- [ ] New SKU: no / appendix PR included
```
...
- Do **not** link `plans/` from public README, PyPI long-description, or plugin market copy  
- Do **not** export charter to public wiki without redaction  
- Contractors: NDA + read-only access to `plans/` on need-to-know basis  
